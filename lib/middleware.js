  ...
  function reauthenticate(req, res) {
    ...
    if (req.method.toLowerCase() === 'get' && !isJSON) {
      req.session.redirectPath =
        // NOTE: the following logic assumes DASHBOARDV2_MOUNT ends with a slash
        req.url !== '/' ?
          process.env.DASHBOARDV2_MOUNT + (req.url[0] === '/' ? req.url.substring(1) : req.url) :
          process.env.DASHBOARDV2_MOUNT.length > 0 ? process.env.DASHBOARDV2_MOUNT : '/';
      console.log('***** node-heroku-bouncer redirecting to: ' + process.env.DASHBOARDV2_MOUNT + 'auth/heroku');
      res.redirect(process.env.DASHBOARDV2_MOUNT + 'auth/heroku');
    } else {
      res.statusCode = 401;
